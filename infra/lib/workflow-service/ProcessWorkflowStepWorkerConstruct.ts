...
export interface IProcessWorkflowStepWorkerConstructProps {
  dynamoDbTable: Table
  eventBus: EventBus
  s3Bucket: Bucket
}
 ...
 *
 */
export class ProcessWorkflowStepWorkerConstruct extends Construct {
  /**
   *
   */
  constructor(scope: Construct, id: string, props: IProcessWorkflowStepWorkerConstructProps) {
    super(scope, id)
    const dlq = this.createProcessWorkflowStepWorkerDlq(scope, id)
    const queue = this.createProcessWorkflowStepWorkerQueue(scope, id, dlq)
    this.createProcessWorkflowStepWorkerFunction(scope, id, props.dynamoDbTable, props.s3Bucket, queue)
    this.createProcessWorkflowStepWorkerRoutingRule(scope, id, props.dynamoDbTable, props.eventBus, queue)
  }

  /**
   *
   */
  private createProcessWorkflowStepWorkerDlq(scope: Construct, id: string): Queue {
   ...
   *
   */
  private createProcessWorkflowStepWorkerFunction(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    s3Bucket: Bucket,
    ...
    const lambdaFunc = new NodejsFunction(scope, lambdaFuncName, {
      ...
      entry: join(servicesRoot, 'src/workflow-service/handlers/processWorkflowStepWorker.ts'),
      handler: 'handler',
      environment: {
   ...
   *
   */
  private createProcessWorkflowStepWorkerRoutingRule(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    eventBus: EventBus,
